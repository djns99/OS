 ...
 *  
 * void OS_Init(void)
 *    The function main() will be called first by your bootstrap code (typically
 *    assembly).  Before any calls
 ...
 *
 * MEMORY MANAGEMENT:
 *    MEMORY is allocated/deallocated dynamically by processes within the system
 *    based on chunks and are serviced in the order that they arrive. In the
 *    event that no memory is available a user is denied allocation. If there
 *    is memory available then the user is allocated the smallest block
 *    available to satisfy the request.
 *    Blocks of memory are organized in powers of 2 with the smallest size
 *    allocated being 2 bytes. However, every block of memory has an additional
 *    2 bytes allocated at the beginning of the block that the user application
 *    does not know to store control information. This control information can
 *    store any data that you deem useful. So, the true minimum size of memory
 *    allocated is 4 bytes.
 *
 * void  OS_InitMemory()
 *    Initialize the memory management sub-system for all processes. Must be
 *    called before any memory can be allocated/deallocated.
 *
 * MEMORY  OS_Malloc( unsigned int val )
 *    Allocated memory of size val bytes if available. If allocated return the
 *    address of the memory to be used. If no memory is available return 0.
 *
 * BOOL  OS_Free( MEMORY m )
 *    Deallocate the memory referenced by m *if* allocated. When released 
 *    perform any memory management operations necessary (de-fragmentation).
 *    Return TRUE if successfully released memory back into memory pool
